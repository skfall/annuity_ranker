...
function download_activity(){
    $.post('split/ajax/heandlers/handle/handle.json.download_activity.php', {}, function(data, status){
        if (status == "success") {
            if (data.status == "success") {
                var download_link = data.download_link;
 
                $('#ajax-getter').html("<a href='"+download_link+"' target='_blank' style='color: #fff; font-size: 25px; opacity: 1; padding-top: 16px; padding-bottom: 10px; display: block;'>DOWNLOAD LINK</a>");
            }
        }
    }, 'json');
    }