<?php 
	...
	$q = "SELECT M.id FROM `osc_companies` AS M WHERE M.alias = '$alias' AND M.id != $item_id LIMIT 1";
	$check_alias = $ah->rs($q);
	if(!$check_alias){
		$name_len = mb_strlen($_POST['name']);

		if ($name_len > 0) {
			$query = "UPDATE `osc_companies` SET ";
			...
			$ah->rs($query);
			$data['message'] = "Company was successfully saved.";
