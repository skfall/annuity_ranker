...
class Ranker extends BasicHelp {
		...
		public function getTexts($params=array(),$typeCount=false) {
			$filter_and = "";
			if(isset($params['filtr']['massive'])) {
				foreach($params['filtr']['massive'] as $f_name => $f_value) {
					if($f_value < 0) continue;
		...
		public function getUserAnswer($id) {
			$query = "
				SELECT M.*, Q.annuity_id, Q.question, A.name as annuity_name, 
				(SELECT `answer` FROM `osc_answers` WHERE `id` = M.answer_id LIMIT 1) as answer 
				FROM `osc_user_answers` as M 
				LEFT JOIN `osc_questions` AS Q ON Q.id = M.question_id 
				LEFT JOIN `osc_annuities` AS A ON A.id = Q.annuity_id 
				WHERE M.id='$id' 
				LIMIT 1
		...
		public function getUserAnswers($params=array(),$typeCount=false) {
			$filter_and = "";
			if(isset($params['filtr']['massive'])) {
				foreach($params['filtr']['massive'] as $f_name => $f_value) {
					if($f_value < 0) continue;
		...
		public function getUserActivities($params=array(),$typeCount=false) {
			$filter_and = "";
			if(isset($params['filtr']['massive'])) {
				foreach($params['filtr']['massive'] as $f_name => $f_value) {
					if($f_value < 0) continue;
