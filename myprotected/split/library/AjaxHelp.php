...
class ajaxHelp
{
	...
	// Send mail
	public function wp_send_letter($to_mail,$from,$subject_mail,$message_mail,$system="KAM STUDIO")
	{
		if($system=="KAM STUDIO") $system = $_SERVER['HTTP_HOST'];
		...
		try
		{
			...
			$message_attach = "<br> <p>Best regards, <a href=\"http://".$_SERVER['HTTP_HOST']."\">".$_SERVER['HTTP_HOST']."</a></p>".$message_date;
			$message = ' 
				<html> 
  					<head> 
  					<title>'.$system.'</title> 
	...
	*/
	...
	// Корректный вывод даты
		...
		public function addWaterMark($file, $mark){
			$image = file_get_contents($file);
			$size = getimagesize($file);
			$watermark = file_get_contents($mark);
			...
			imagejpeg($a, $file_path.$file_upload);
			imagedestroy($a);
			imagedestroy($b);
		}
		...
		public function getUsersData($params=array(),$typeCount=false){
			...
			if(!$typeCount)
			{
				...
				$query = "SELECT M.* 
						...
						FROM [pre]client_data_storage as M  
	...
	// SITEMAP GENERATION
	// NEW LIFE WITHOUT COMMENTARIES AND MARGINS....

	public function getNewsUrls(){
        $q = "SELECT  M.id AS url, M.caption AS name FROM `osc_page_news_items` AS M WHERE M.block = 0 ORDER BY M.id DESC LIMIT 1000";
        return $this->rs($q);
    }

    public function getMenuNameByTable($table, $id = ""){
        $id = (int)$id;
        $id_row = "";
    ...
    public function getEventNameByType($type){
        $q = "SELECT M.name FROM `osc_sys_events` AS M WHERE M.type = '$type' LIMIT 1";
        $resultMass = $this->rs($q);
		if($resultMass) $result = $resultMass[0];
        return $result;
    }
